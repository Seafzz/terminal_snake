...
# Instructions for the player if they answered no
...
def main(stdscr):
    ...
    while True:
        ...
        if (snake[0][0] in [0, sh] or snake[0][1] in [0, sw] or
                snake[0] in snake[1:]):
        ...
        # Refresh the screen
        w.clear()  # Clear the screen before re-drawing
        for y, x in snake:
            w.addch(y, x, curses.ACS_CKBOARD)
        w.addch(int(food[0]), int(food[1]), curses.ACS_PI)
        w.refresh()  # Refresh the window with updated positions
